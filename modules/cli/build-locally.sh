...
function generate_galasactl_documentation {
    ...
    # Call the documentation generator, which builds .md files into the git source folder, so it can be published on git directly.
    # We can remove this when we no longer depend upon it.
    call_docs_generator "${machine}" "${architecture}" "${BASEDIR}/docs/generated"
...
function call_docs_generator {
    machine="$1"
    architecture="$2"
    generated_docs_folder="$3"

    h2 "Generating CLI docs : Documentation will be placed in ${generated_docs_folder}"
